	...
	std::vector<T_NodeType*> AStar<T_NodeType, T_ConnectionType>::FindPath(T_NodeType* pStartNode, T_NodeType* pGoalNode)
	{
		...
		while (!openList.empty())
		{
			...
			for (const auto& con : m_pGraph->GetNodeConnections(currentRecord.pNode->GetIndex()))
			{
				...
				bool isValid{ true };
				{
					//for (const auto& closedNR : closedList)
					//{
					//	if (closedNR.pNode == nextNode)
					//	{
					//		if (G <= closedNR.costSoFar)
					//		{
					//			closedList.erase(closedNR);

					//			isValid = !isValid;
					//		}
					//	}
					//}
					//for (const auto& opendNR : openList)
					//{
					//	if (opendNR.node == nextNode)
					//	{
					//		if (G <= opendNR.costSoFar)
					//		{
					//			openList.erase(openList.find(opendNR));
					//			isValid = !isValid;
					//		}
					//	}
					//}

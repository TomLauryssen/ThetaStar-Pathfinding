...
namespace Elite
{
	...
	private:
		...
		void AddNodesToPath(vector<T_NodeType*>& path, std::shared_ptr<Elite::IGraph<Elite::GraphNode2D, Elite::GraphConnection2D>>& graphCopy, int idx) const;
	...
	inline vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		AddNodesToPath(path, graphCopy, startIdx);
	...
	template<class T_NodeType, class T_ConnectionType>
	inline void EulerianPath<T_NodeType, T_ConnectionType>::AddNodesToPath(vector<T_NodeType*>& path, std::shared_ptr<Elite::IGraph<Elite::GraphNode2D, Elite::GraphConnection2D>>& graphCopy, int idx) const
	{
		if (graphCopy->GetNodeConnections(idx).size() == 0)
		{
			path.push_back(graphCopy->GetNode(idx));
		}
		else if (graphCopy->GetNodeConnections(idx).size() > 0)
		{
			path.push_back(graphCopy->GetNode(idx));
			
			int nextNode = graphCopy->GetNodeConnections(idx).front()->GetTo();
			
			graphCopy->RemoveConnection(idx, nextNode);

			AddNodesToPath(path, graphCopy, nextNode);
		}
	}
}